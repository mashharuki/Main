...
本ドキュメントでは、ハッカソンで実装可能なMVP（Minimum Viable Product）の要件を定義します。
...
1. 研究者がクエリを送信したとき、スマートコントラクトは、登録されたすべての患者データに対してクエリを実行しなければならない
2. スマートコントラクトは、集計統計（例：平均年齢、性別分布、症例の有病率）のみを返さなければならない
3. スマートコントラクトは、個別の患者データを決して露出してはならない
4. スマートコントラクトは、クエリが正しく実行されたことを示すゼロ知識証明を生成しなければならない
5. クエリが実行されたとき、スマートコントラクトは、クエリメタデータ（研究者ID、タイムスタンプ、クエリタイプ）を記録しなければならない
...
1. 研究者がAIモデルを実行のために送信したとき、スマートコントラクトは、登録されたすべての患者データに対してモデルを実行しなければならない
2. スマートコントラクトは、生データを露出することなく、分析結果（例：相関係数、リスクスコア）を返さなければならない
...
**ユーザーストーリー:**   
患者として、登録されたデータを表示したい。  
そうすることで、自分の情報がプラットフォームに正しく登録されていることを確認できる。

#### 受入基準

1. フロントエンドは、患者の登録された医療データ（年齢、性別、症例）を表示しなければならない
2. フロントエンドは、データ登録の確認状況を表示しなければならない
3. フロントエンドは、データが研究に使用された回数の統計を表示しなければならない
4. フロントエンドは、データの更新機能を提供しなければならない
...
1. フロントエンドは、利用可能なデータセットに関する統計（総患者数、データタイプ）を表示しなければならない
...
1. バックエンドは、患者登録、データクエリのためのAPIエンドポイントを提供しなければならない
