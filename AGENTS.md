# NextMed: AI医療データプラットフォーム AI駆動開発 共通ガイドライン

## 全体的な方針
- 特段の指定がない限り、わかりやすくて自然な日本語で回答を出力してください
- 毎回必ずプロジェクトをアクティベートしてください。
- プロジェクトがオンボーディングされていない場合はオンボーディングしてください。
- .kiro/specs 配下にはハッカソンで作るプロダクトの概要、要件定義書、タスクリストが格納されています。実装の際には必ず参照してください。
- references/helixchain には 2025年9月にmidnightのハッカソンで受賞したチームのコードが格納されています。実装例として参考にしてください。
